...
# Import function for parallelized VirFinder
source("/hps/nobackup2/production/metagenomics/garp/software/parVF.pred_prok.R")
...
option_list <- list(
  ...
  make_option(c("-n", "--ncore"), type="integer", default=8,
              help="number of cores to compute (default: 8)", metavar="ncore"))
...
# run VirFinder
cat("Running VirFinder on:",opt$fasta,"using",as.character(opt$ncore),"cores","\n")
predResult <- parVF.pred(path, cores = opt$ncore)
