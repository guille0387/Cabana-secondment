...
sys.path.insert(-1, "/homes/garp/scripts/viral_taxonomic_pipeline")

from vphmm_taxa_dict import taxa_dict
...
def ratio_evalue(vphmm_df):
	...
	informative_df = vphmm_df[(vphmm_df["target name"].isin(taxa_dict.keys())) & (vphmm_df["i-Evalue"] <= 0.01)]
	if len(informative_df) < 1:
		...
		for vphmm in vphmm_hits:
			...
			more_than_one = list(vphmm_specific_df["query name"].value_counts()[vphmm_specific_df["query name"].value_counts() > 1].index)
			query_list = []
			for i in range(len(vphmm_specific_df)):
				if vphmm_specific_df["query name"][i] not in query_list:
					...
					if vphmm_specific_df["query name"][i] in more_than_one:
						number = vphmm_specific_df["query name"].value_counts()[vphmm_specific_df["query name"][i]]
						coords_list = sorted([(vphmm_specific_df["hmm coord from"][i + j], vphmm_specific_df["hmm coord to"][i + j]) for j in range(number)])
						reference_pair = list(coords_list[0])
						final_coords_list = []
						for elem in range(1, len(coords_list)):
							if coords_list[elem][0] <= reference_pair[1]:
								reference_pair[1] = max(reference_pair[1], coords_list[elem][1])
							else:
								final_coords_list.append(tuple(reference_pair))
								reference_pair[0] = coords_list[elem][0]
								reference_pair[1] = coords_list[elem][1]
						final_coords_list.append(tuple(reference_pair))
						total_hmm_align = sum([final - initial + 1 for initial,final in final_coords_list])
						hmm_ratio = total_hmm_align/vphmm_specific_df["tlen"][i]
					else:
						hmm_ratio = (vphmm_specific_df["hmm coord to"][i] - vphmm_specific_df["hmm coord from"][i] + 1)/vphmm_specific_df["tlen"][i]
					final_hit_list.append((vphmm, vphmm_specific_df["query name"][i], hmm_ratio, abs(int(("%E" % decimal.Decimal(vphmm_specific_df["full sequence E-value"][i])).split("E")[-1]))))
		final_df = pd.DataFrame(final_hit_list, columns = ["ViPhOG", "query", "Ratio", "Abs_Evalue_exp"])
		final_df["Abs_Evalue_exp"] = final_df["Abs_Evalue_exp"].apply(lambda x: 277 if x == 0 else x)
