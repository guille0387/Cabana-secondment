...
if __name__ == "__main__":
	...
	parser.add_argument("--prop", dest = "prot_prop", type = float, help = "Minimum proportion of proteins in a contig that must have a ViPhOG annotation in order to provide a taxonomic assignment (default: 0.2)", default = 0.2)
	parser.add_argument("--taxthres", dest = "tax_thres", type = float, help = "Minimum proportion of annotated genes required for taxonomic assignment (default: 0.6)", default = 0.6)
	...
	if len(sys.argv) == 1:
		...
		output_df = contig_tax(input_df, args.min_prot, args.prot_prop, args.tax_thres)
		output_df.to_csv(os.path.join(args.outdir, re.split(r"\.[a-z]+$", os.path.basename(args.input_file))[0] + "_tax_assign_%s_%s.tsv" % (int(args.prot_prop * 100), int(args.tax_thres * 100))), sep = "\t", index = False)
